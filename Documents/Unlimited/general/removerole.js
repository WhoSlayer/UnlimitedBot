...
module.exports.run = async (bot, message, args) => {
    ...
    const selectRole = new Discord.MessageEmbed().setDescription("<:unsuccess:711232495592538124> | **Please provide a user to remove a role to.**").setColor('RED')
    if(!rMember) return message.channel.send(selectRole)
    ...
    if(rMember.roles.cache.has(role)) {
        let hasRole = new Discord.MessageEmbed().setDescription(`<:unsuccess:711232495592538124> | **${rMember.user.tag}** doesn't have that role!`).setColor('RED')
