...
exports.run = (client, message, args) => {
  ...
  if(!message.member.hasPermission("BAN_MEMBERS") || !message.guild.owner) return message.channel.send("You do not have permission to use this command.");
...
if (reason.length < 1) return message.channel.send(noreason);
  ...
  const bannn = new Discord.MessageEmbed()
    .setColor("#03adfc")
    .setDescription(`You were banned from ${message.guild.name} | ${reason} | \n You may appeal your ban, if you show honesty and regret for your actions is given. - https://forms.gle/G5ex2qye4FWbakuT8 ` )
    .setTimestamp()
...
.setColor("#2dfa78")
.setDescription(`\`\`\`${message.author.tag} has swong the ban hammer on ${user.user.tag} | ${reason}\`\`\``)
